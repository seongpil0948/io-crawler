...
def export_vendor_prod(file_path: str):
    ...
    print("start export vendor prod: ", file_path, fire_app)
    ...
    print("target garment length: ", len(garments))
    ...
    for idx, g in tqdm(enumerate(garments)):
        ...
        if idx % 100 == 0:
            batch.commit()
    batch.commit()
    print("set count: ", set_cnt)
    print("total count: ", idx + 1)
...
if __name__ == "__main__":
    ...
    for f_name in os.listdir(OUT_PATH):
        if os.path.splitext(f_name)[1] == ".pkl" and f_name not in state["done_files"]:
            ...
            print(f"{f_name} is skipped export target")

    print("target_files", target_fs)
    for target_file in target_fs:
        file_path = os.path.join(OUT_PATH, target_file)
        export_vendor_prod(file_path)
